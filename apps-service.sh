...
az storage account create --name rifatappstorage --sku Standard_LRS --resource-group $mygroup --kind blobStorage --location southcentralus --access-tier Hot

# Creating a container for blob with storage account key
az storage container create --name rblobcontainer --public-access blob --account-key 861bxwKuPwKIzdXKFL4r49xTiapvzpZoS5c5YYrAqcZy+v9Y7iiy/4NZ2fcPXyEsjm82XhhEAgg+WDOQTvRkNA== --account-name rifatappstorage

# Creating an App Service plan in `FREE` tier.
...
# Configuring continuous deployment from GitHub.
